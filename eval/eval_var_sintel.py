# -*- coding: utf-8 -*-
# @Time    : 17-10-3 上午10:11
# @Author  : Fei Xue
# @Email   : feixue@pku.edu.cn
# @File    : eval_var_sintel.py
...
def var_mean(flow_to_mean):
    ...
    def _var_mean(flow_to_mean):
        ...
        var1 = np.zeros(var_mea.shape, np.float32)
        var1 = var_mea
        return [flow_to_mean, var_mea, var1, var_img]

    solved_data = tf.py_func( _var_mean, [flow_to_mean], [tf.float32, tf.float32, tf.float32, tf.float32], name='flow_mean')
    mean, var, var1, var_img = solved_data[:]

    mean = tf.squeeze(tf.stack(mean))
    var = tf.squeeze(tf.stack(var))
    var1 = tf.squeeze(tf.stack(var1))
    var_img = tf.squeeze(tf.stack(var_img))

    mean.set_shape(list(FLAGS.d_shape_flow))
    var.set_shape(list(FLAGS.d_shape_flow[:2])+ [1])
    var1.set_shape(list(FLAGS.d_shape_flow[:2])+ [1])
    var_img.set_shape(list(FLAGS.d_shape_img))
    return mean, var, var1, var_img
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        imgs_0_rs = tf.image.resize_nearest_neighbor(imgs_0, FLAGS.img_net_shape[:2])
        imgs_1_rs = tf.image.resize_nearest_neighbor(imgs_1, FLAGS.img_net_shape[:2])
        print(imgs_0, imgs_1, "A_input", flows)
        # img summary after loading
        flownet.image_summary(imgs_0, imgs_1, "A_input", flows)

        # Get flow tensor from flownet model
        calc_flows = architectures.flownet_dropout(imgs_0_rs, imgs_1_rs, flows)

        # calc mean and confidence images
        flow_mean, confidence, confidence1, conf_img  = var_mean(calc_flows)
        ...
        #solved_flow_c1 = flownet.bil_solv_var(img_0, flow_mean, confidence1, flow)
        #aee_bs_c1 = aee_f(flow, solved_flow_c1, var_num)
