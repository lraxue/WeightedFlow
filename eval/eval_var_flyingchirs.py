# -*- coding: utf-8 -*-
# @Time    : 17-10-3 上午10:10
# @Author  : Fei Xue
# @Email   : feixue@pku.edu.cn
# @File    : eval_var_flyingchirs.py
...
flags.DEFINE_string('dataset', '/flownet_s', 'Data Set in use')

flags.DEFINE_string('grid_params', {'sigma_luma': 4, 'sigma_chroma': 4, 'sigma_spatial': 2},
                    'grid_params')
...
def slice_vector(vec, size):
    x = tf.slice(vec, [0, 0, 0, 0], [size] + FLAGS.img_net_shape[:2] + [1])
    y = tf.slice(vec, [0, 0, 0, 1], [size] + FLAGS.img_net_shape[:2] + [1])
...
def var_mean(flow_to_mean):
    ...
    def _var_mean(flow_to_mean):
        ...
        flow_to_mean = np.zeros(list(FLAGS.flow_net_shape), np.float32)
        flow_to_mean[:, :, 0] = flow_x_m
        flow_to_mean[:, :, 1] = flow_y_m
        var_img = np.zeros(list(FLAGS.img_net_shape), np.float32)
        var_img[:, :, 0] = var_mea
        var_img[:, :, 1] = var_mea
        var_img[:, :, 2] = var_mea
    ...
    mean.set_shape(list(FLAGS.flow_net_shape))
    var.set_shape(list(FLAGS.flow_net_shape[:2]) + [1])
    var_img.set_shape(list(FLAGS.img_net_shape))
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        var_num = 1
        img_0, img_1, flow = flownet_tools.get_data_flow_s(FLAGS.datadir, False, var_num)
